...
(function() {
    ...
    addLoadEvent(function() {
        ...
        //use-themed-js formelement.getAttribute('data-usethemedjs')
        var body = document.getElementsByTagName("body")[0],
            themedjs = body.getAttribute('data-usethemedjs');

        if (themedjs !== null && themedjs !== undefined){
            return false;
        }
        ...
        forEach(legends, function(legend) {
            ...
            var fsid = 'general';
            var row = getFirstElementByTagAndClassName('div', 'html', legend.parentNode);
            if (row) {
		...
		appendChildNodes(formid,
			tabDIV, DIV({'class': 'tabbed-fieldsets subpage'}, fieldsets), getFirstElementByTagAndClassName('input', null, formid + '_submit_container'));
        removeElement(
            getFirstElementByTagAndClassName('div', null, formid)
        );
