...
{if $task->description}
                ...
                {if $task->tags}<p class="tags"><strong>{str tag=tags}:</strong> {list_tags owner=$task->owner tags=$task->tags}</p>{/if}</div></td>
{else}
...
{if $task->description}
                <td class="plantasktitledescript"><a class="task-title" href="">{$task->title}</a>
                <div class="task-desc hidden" id="task-desc-{$task->id}">
